...
public class ConstFold implements ast.Visitor {
  ...
  public void visit(Add e) {
    ...
    if (left instanceof Num && right instanceof Num) this.exp = new Num(((Num)left).num + ((Num)right).num, e.pos);
    else this.exp = new Add(left, right, e.pos);
  }

  @Override
  public void visit(And e) {
    // do nothing since no opt can be accept here
    this.exp = new And(e.left, e.right, e.pos);
  }
  ...
  public void visit(Length e) {
    e.array.accept(this);
    Ast.Exp.T array = this.exp;
    this.exp = new Length(array, e.pos);
  }
  ...
  public void visit(Not e) {
    e.exp.accept(this);
    Ast.Exp.T exp = this.exp;
    if (exp instanceof False) this.exp = new True(e.pos);
    else if (exp instanceof True) this.exp = new False(e.pos);
    else this.exp = new Not(exp, e.pos);
  }
